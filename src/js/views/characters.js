                    ...
                    return (
                        <div key={item.url} className="card text-start mx-3 my-3 p-0 carditem" style={{width: '400px'}}>
                            ...
                            <div className="card-body">
                                ...
                                <button onClick={()=>actions.addFavorite1(item.name, index)} className="btn btn-outline-warning float-end" style={{backgroundColor: store.favorites.includes(item.name) ? "#ffc107" : "", color: store.favorites.includes(item.name) ? "black" : "" }} >♡</button>
                            </div>
                        </div>
                    );
                })}
